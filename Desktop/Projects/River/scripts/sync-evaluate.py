...
def load_evaluation_events() -> dict:
    """Load the evaluation event configuration."""
    config_path = os.path.join(
        os.path.dirname(__file__), "..", "config", "evaluation-events.json"
    )
...
def main():
    ...
    # Check if task_type requires sync evaluation
    events = load_evaluation_events()
    sync_types = [c["task_type"] for c in events.get("sync_evaluation", {}).get("trigger_conditions", [])]
    if trace["task_type"] not in sync_types:
        print(f"  Task type '{trace['task_type']}' does not require sync evaluation. Skipping.")
        sys.exit(0)
