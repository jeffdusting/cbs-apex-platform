...
def main():
    ...
    # Load evaluation events config for task_type routing (IV#7/RA.11)
    try:
        events_config = evaluator.load_events_config()
    except Exception as e:
        print(f"ERROR loading evaluation-events.json: {e}")
        sys.exit(2)
    ...
    skipped_self_check = 0
    for i, trace in enumerate(scorable, 1):
        trace_short = trace["id"][:8]
        mode = evaluator.resolve_evaluation_mode(trace["task_type"], events_config)
        if mode == "self_check":
            print(
                f"  [{i}/{len(scorable)}] {trace_short} task={trace['task_type']} "
                f"— self_check only, no external evaluation"
            )
            skipped_self_check += 1
            continue
        try:
    ...
    print(
        ...
        f"{skipped_self_check} skipped (self_check only)"
    )
