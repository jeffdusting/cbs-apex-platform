#!/usr/bin/env python3
"""
WR KB Email Intake — add content to the WaterRoads knowledge base by email.

How it works:
  1. Jeff or Sarah send an email to jeff@cbs.com.au with subject starting "WR KB:"
...
Subject line format:
  WR KB: <category> | <optional title override>

Examples:
  "WR KB: Governance | Board resolution template 2026"
  ...
  "WR KB: Reference/Shipley | Win Themes Guide"
...
SUBJECT_PREFIX = "WR KB"  # matches "WR KB:", "WR KB -", "WR KB Items" etc.
...
def extract_urls(text):
    ...
    for url in raw_urls:
        ...
        if any(p.search(url) for p in URL_FOLLOW_RE):
            follow.append(url)
        # Also follow ANY url the user explicitly put in a "WR KB:" email —
...
def supa_url():
    return os.environ["WR_SUPABASE_URL"]


# ---------- Subject parsing ----------
def parse_subject(subject):
    """Parse 'WR KB: category | title' or 'WR KB category' → (category, title_override).
    Tolerant of missing colon — if someone writes 'WR KB Items' it still works."""
    if not subject: return "incoming", None
    # Strip the prefix (with or without colon)
    rest = subject
    for prefix in ["WR KB:", "WR KB -", "WR KB"]:
...
def find_kb_emails(token, since_iso):
    """Find emails with subject starting 'WR KB'. Uses processed ID tracking
...
def main():
    ...
    print(f"=== WR KB Email Intake — {datetime.now(timezone.utc).isoformat()} ===")
    ...
    if total_files > 0:
        notify(
            f"WR KB - {total_files} documents ingested via email",
