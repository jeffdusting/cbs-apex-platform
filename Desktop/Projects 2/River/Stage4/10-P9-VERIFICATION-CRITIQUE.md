...
Check all 9 tables exist with expected columns. Verify `ca_send_approved` column on `tender_register`. Test `match_documents` with `match_threshold`. Verify active rubric exists. Check no NULL entities in `documents`. Confirm post-dedup row count is within `1,000–5,000` rows (not 15,655) — the actual count after S4-P4 dedup is 1,273, which is within this range.
...
- WR agents are configured so that `SUPABASE_URL` resolves to the WR Supabase project at runtime (the agent's environment maps `SUPABASE_URL` to the WR project's URL — agents do not need a separate `WR_SUPABASE_URL` variable)
