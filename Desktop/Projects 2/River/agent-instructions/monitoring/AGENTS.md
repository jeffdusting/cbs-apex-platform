...
### 7. Trace Reconciliation (RA.5)

Run `scripts/trace-reconciliation.py --hours 24` to detect agents emitting outputs without writing traces. If the Paperclip session cookie is current, the script compares trace counts against observed agent activity; otherwise it falls back to heartbeat-expected frequency (a 2h heartbeat should produce ~12 traces per day).

Flag any agent with divergence > 5% or zero traces where >0 were expected. Include a **"self-check divergence"** metric in the digest (see Step 8) — it is computed by `sync-evaluate.py` and stored in the evaluation_scores row metadata.

### 8. Compose Daily Digest
...
- {divergence_count} self-check divergences (>1.0 gap between agent self-score and evaluator)
- Trace reconciliation: {recon_status}
